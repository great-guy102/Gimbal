...
void PowerLimiter::PwrLimitCalcSpd(const MotorRuntimeParamsList &motor_run_par_list, float *limited_spd_ref_radps)
{
  ...
  motor_num_ = this->motor_run_par_list_.size();
  if (last_act_ref_spd_ == nullptr) {
    last_act_ref_spd_ = hello_world::Allocator<float>::allocate(motor_num_);
  }
  if (spd_ref_radps_record_ == nullptr) {
    spd_ref_radps_record_ = hello_world::Allocator<float>::allocate(motor_num_);
  }
  ...
  do {
    ...
    if (pwr_limit_calc_data_.p_prediction < pwr_limit_calc_data_.p_ref) {
      ...
      uint8_t i = 0;
      for (auto tmp = motor_run_par_list_.begin(); tmp != motor_run_par_list_.end(); ++tmp, i++) {
        spd_ref_pow = spd_measure_pow = spd_measure_ref = constant = 0;
        /*根据type决定使用电机的静态参数*/
        ...
        /*按照输入顺序记录各轮子的期望速度，用于后续优化计算*/
        spd_ref_radps_record_[i] = tmp->spd_ref_radps;
        ...
        beta += motor_km * (ka + kv) * spd_ref_pow - 2 * motor_R * (ka * kv + ka * ka) * spd_measure_ref;
        gamma += motor_R * ka * ka * spd_measure_pow - motor_km * ka * spd_measure_ref;
      }
      constant = pwr_limit_static_params_.p_bias;
      ...
      tmp1 = tmp2 = 0;
      /*对于速度和限制系数的连续性优化,保证速度连续或者快速抑制*/
      for (uint8_t i = 0; i < motor_num_; i++) {
        tmp1 += last_act_ref_spd_[i] * spd_ref_radps_record_[i];
        tmp2 += spd_ref_radps_record_[i] * spd_ref_radps_record_[i];
      }
      float k_limit_ideal = pwr_limit_calc_data_.k_limit;
      k_limit_ideal = hello_world::Bound(k_limit_ideal, 0, 1);
      float tmp_a = 0;
      tmp_a = 15000 * (1 - k_limit_ideal) * (1 - k_limit_ideal);
      pwr_limit_calc_data_.k_limit = (tmp1 + tmp_a * k_limit_ideal) / (tmp2 + tmp_a);
      /*对于削减系数的突变进行处理*/
      alpha = beta = gamma = 0;
    }
  } while (0);
  ...
  uint8_t cnt = 0;
  for (auto tmp = motor_run_par_list_.begin(); tmp != motor_run_par_list_.end(); ++tmp, cnt++) {
    limited_spd_ref_radps[cnt] = pwr_limit_calc_data_.k_limit * tmp->spd_ref_radps;
    last_act_ref_spd_[cnt] = limited_spd_ref_radps[cnt];
  }
}

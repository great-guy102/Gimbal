...
hw_module::Fric::Config kFricConfig = {
    .min_reasonable_blt_spd = 14.0, // 最小合理弹丸速度, >0, 无默认值, m/s,
                                    // 小于该值认为裁判系统反馈数据错误
    .max_reasonable_blt_spd = 30.0, // 最大合理弹丸速度, >0, 无默认值, m/s,
                                    // 大于该值认为裁判系统反馈数据错误
    .min_target_blt_spd = 19.0, // 弹丸速度期望值区间下限, >0, 无默认值, m/s
    .max_target_blt_spd = 20.0, // 弹丸速度期望值区间上限, >0, 无默认值, m/s
    .default_spd_ref = 733.3, // 摩擦轮期望速度预设值, >0, 无默认值, rad/s
...
hw_module::Fric *GetFric() {
  ...
  if (!is_fric_created) {
    unique_fric.registerPid(GetPidMotorFricLeft(), 0); // 注册MultiNodesPID指针
    unique_fric.registerPid(GetPidMotorFricRight(), 1); // 注册MultiNodesPID指针
    unique_fric.registerMotor(GetMotorFricLeft(), 0);  // 注册电机指针
    unique_fric.registerMotor(GetMotorFricRight(), 1); // 注册电机指针

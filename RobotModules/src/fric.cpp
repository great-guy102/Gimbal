...
void Fric::updateMotorData() {
  ...
  FricStatus::FricIdx fi_fric[2] = {FricStatus::kFricIdxLeft,
                                    FricStatus::kFricIdxRight};
  for (size_t i = 0; i < 2; i++) {
    motor_ptr = motor_ptr_[mi_frics[i]];
    HW_ASSERT(motor_ptr != nullptr, "ptr to fric motor %d is nullptr",
              mi_frics[i]);
...
void Fric::updateRfrData(const FricRfrInputData &inp_data) {
  if (rfr_data_.bullet_speed != inp_data.bullet_speed) {
    ...
    if (inp_data.bullet_speed >= cfg_.min_bullet_speed &&
        inp_data.bullet_speed <= cfg_.max_bullet_speed) {
      rfr_data_.last_bullet_speed = rfr_data_.bullet_speed;
      rfr_data_.bullet_speed =
          rfr_data_.last_bullet_speed * 0.5f + inp_data.bullet_speed * 0.5f;
    }
  }
  // 更新当前数据
...
void Fric::updateMotorFricStuckStatus() {
  if (fric_status_.cur_fdb[FricStatus::kFricIdxLeft] <=
          -cfg_.fric_stuck_curr_thre ||
      fric_status_.cur_fdb[FricStatus::kFricIdxRight] <=
          -cfg_.fric_stuck_curr_thre) {
  ...
  } else if (fric_status_.cur_fdb[FricStatus::kFricIdxLeft] >=
                 cfg_.fric_stuck_curr_thre ||
             fric_status_.cur_fdb[FricStatus::kFricIdxRight] >=
                 cfg_.fric_stuck_curr_thre) {
    fric_status_.stuck_duration += interval_ticks_;
...
void Fric::updateMotorFricHoldStatus() {
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    float spd_fdb_delta =
        fric_status_.spd_fdb[i] - fric_status_.last_spd_fdb[i];
    float spd_err = fric_status_.spd_ref - fric_status_.spd_fdb[i];
    hold_flag[i] = fabsf(spd_fdb_delta) < fabsf(cfg_.fric_spd_delta_thre) &&
                   fabsf(spd_err) < fabsf(cfg_.fric_spd_err_thre);
  }
...
void Fric::calcFricSpdRefOnResurrection() {
  ...
  if (working_mode_ == ShooterWorkingMode::Stop ||
      fric_status_.stuck_status != StuckStatus::None) {
...
void Fric::calcFricSpdRefOnWorking() {
  ...
  // 正常模式、嗜血模式下，摩擦轮的转速正常
  // 当摩擦轮卡住时，摩擦轮停转，防止电机堵转并出现损坏
  if (working_mode_ == WorkingMode::Stop ||
      fric_status_.stuck_status != StuckStatus::None) {
    fric_status_.spd_ref = 0.0f;
    return;
  }
...
void Fric::calcMotorFricInput() {
  ...
  FricStatus::FricIdx fi_frics[2] = {FricStatus::kFricIdxLeft,
                                     FricStatus::kFricIdxRight};
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    HW_ASSERT(motor_ptr != nullptr, "pointer to Fric %d Motor is nullptr",
              mi_frics[i]);
    HW_ASSERT(pid_ptr_ != nullptr, "pointer to Fric %d PID is nullptr",
              pi_frics[i]);
    ...
    if (motor_ptr->isOffline()) {
      ...
      float fdb[2] = {fric_status_.spd_fdb[fi_frics[i]],
                      fric_status_.cur_fdb[fi_frics[i]]};
